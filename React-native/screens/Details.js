...
import { useUpdateInventoryItemMutation } from "../redux/services";
...
const Details = ({ route, navigation }) => {
  const { category_id, item_id, name, price, qty, image, prevScreen } =
    route.params;

  const [updatItem] = useUpdateInventoryItemMutation();

  const [pickedImage, setPickedImage] = useState();
  const [isLoading, setIsLoading] = useState(false);

  const [data, setData] = useState({ name, quantity: qty, price });
  ...
  async function handleUpdate() {
    ...
    updatItem({
      ...
      id: item_id,
      name: data.name,
      image: pickedImage ? response : image,
      qty: data.quantity,
      price: data.price,
      category_id,
    })
